...
all_data_list <- lapply(names(required_cluster_cols), function(df_name) {
  ...
  # Handle nested column names like 'model$predclass'
...
# Standardise country names in your data
...
generate_forecast_data <- function(proportions_data, target_species, target_region, n_forecast_years = 5, n_bootstrap = 100) {
  ...
  for (target_var in target_variables) {
    ...
    initial_model <- xgboost(data = as.matrix(modeling_df[, predictor_variables]), label = modeling_df[[target_var]], nrounds = 100, objective = "reg:squarederror", verbose = 1)
    ...
    for (i in 1:n_bootstrap) {
      ...
      bootstrap_model <- xgboost(data = as.matrix(modeling_df[, predictor_variables]), label = bootstrap_target, nrounds = 100, objective = "reg:squarederror", verbose = 1)
...
ui <- fluidPage(
  ...
  navbarPage("AMR Dashboard",
             ...
             # UPDATED: Global Overview Tab
             
             tabPanel("Global Overview",
                      
                      # ADDED: A single row for shared filters like the year slider
                      
                      fluidRow(
                        
                        column(12, wellPanel(
                          
                          h4("Shared Map Filters"),
                          
                          # This UI is rendered on the server to get the year range from the data
...
server <- function(input, output, session) {
  ...
  # === Global Overview Tab: Server Logic (UPDATED) 
  
  output$year_slider_global <- renderUI({
    
    year_range <- range(all_data_final$Year, na.rm = TRUE)
    
    sliderInput("year_range_global", "Select Year Range:", 
  ...
  map_data_global_cluster <- reactive({
    ...
    req(input$species_global_cluster, input$year_range_global) # Uses shared year_range_global
    
    summary_data <- all_data_final %>% 
      
      filter(Species == input$species_global_cluster, Year >= input$year_range_global[1], Year <= input$year_range_global[2]) %>% 
  ...
  map_data_global_resistance <- reactive({
    ...
    req(input$species_global_res, input$drug_choice_global_res, input$year_range_global) # Uses shared year_range_global
    ...
    summary_data <- all_data_final %>% 
      ...
      filter(
        ...
        Year >= input$year_range_global[1], Year <= input$year_range_global[2], # Filter by year
  ...
  # UPDATED: Plot subtitle now uses the shared year slider input
  ...
  # UPDATED: Plot subtitle now ALSO uses the shared year slider input
  ...
  # === Phenotype Resistance Trends Tab: Server Logic ===
  ...
  output$forecast_plot <- renderPlot({
    ...
    if (is.null(results$forecast_summary)) {
      ...
      return(ggplot() + labs(title = results$log) + theme_void()) # Show error log on plot
